...
\begin{scope}[shift={(6.6,0,0)}]
  \fill[black!40] (0,0,3.4) circle (0.9pt); \node[right, font=\footnotesize] at (0.15,0,3.4) {collocation ($\mathcal{L}_\mathrm{pde}$)};
  \fill[blue!70] (0,0,2.5) circle (1.0pt); \node[right, font=\footnotesize] at (0.15,0,2.5) {$t{=}0$ ($\mathcal{L}_\mathrm{ic}$)};
  \node[star, star points=5, star point ratio=2.3, fill=red!85, inner sep=1.1pt] at (0,0,1.6) {}; \node[right, font=\footnotesize] at (0.15,0,1.6) {gauges ($\mathcal{L}_\mathrm{data}$)};
\end{scope}
\end{tikzpicture}
...
\node[loss, below=9mm of ad] (lic) {$\mathcal{L}_\mathrm{ic}$ (+ $\mathcal{L}_\mathrm{data}$)};
...
\draw[->] (lpde)--(tot);
\draw[->] (lic) -| (tot);
\draw[->, dashed, red!65] (tot.south) |- ($(net.south)+(0,-6mm)$)
  node[glab, midway, below] {backprop $\nabla_\theta\mathcal{L}$} -- (net.south);
\end{tikzpicture}}
\caption{\new{Strong-form PINN: outputs are differentiated to form the SWE residual, and the residual, initial-condition, and optional data terms make the loss.}}

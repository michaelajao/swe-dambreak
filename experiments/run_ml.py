"""PINN comparison: strong-form PINNs (primitive/conservative) and the
FVM-informed PINN, on the same benchmarks and metrics as the classical sweep.

Config-driven (configs/pinn_smoke.yaml for a fast pipeline check, configs/
pinn.yaml for publication settings). Produces per-run artifacts under
runs/ml/<entry>/<benchmark>/seed<k>/ and this run's table in
reports/ml_runs/<config>_table.md, which appends neural rows to a classical
baseline computed on the identical evaluation grid. The curated write-up in
reports/pinn_comparison.md is maintained by hand and is never overwritten here.

Usage: python experiments/run_ml.py [configs/pinn_smoke.yaml]
"""
...
def main() -> None:
    cfg_path = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "configs" / "pinn_smoke.yaml"
    ...
    for bspec in cfg["benchmarks"]:
        ...
        for entry in cfg["entries"]:
            ...
            for seed in cfg["seeds"]:
                ...
                # Record what the directory name alone cannot say: which entry,
                # which benchmark, and the gauge count actually resolved for it.
...
def write_report(cfg, results, classical, device, cfg_path):
    ...
    Deliberately NOT reports/pinn_comparison.md: that file is the curated
    write-up (the publication numbers, the cross-config synthesis, and the
    interpretation prose), and it is not reproducible from any single config.
    Auto-writing it meant a 3-iteration smoke run silently replaced the whole
    analysis with its own twelve lines. One file per config, named after it, so
    a run can never clobber another run's numbers or the curated document.
    """
    L: list[str] = []
    L.append("# PINN comparison (PINNs vs classical)\n")
    ...
    L.append(f"Settings: iters={cfg['train']['iters']}, seeds={cfg['seeds']}. "
             "NOTE: increase both for publication (see configs/pinn.yaml).\n")
